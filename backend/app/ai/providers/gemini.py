...
class GeminiProvider:
    ...
    async def get_info(self) -> AgentInfo:
        ...
        return AgentInfo(
            id=self.id,
            name="Google Gemini",
            description="Облачная модель Google Gemini",
            model=settings.gemini_model,
            provider="gemini",
            available=has_key,
            unavailable_reason=None if has_key else "Укажите GEMINI_API_KEY в .env",
        )
    ...
    async def chat(self, messages: List[ChatMessage]) -> str:
        if not settings.gemini_api_key.strip():
            raise HTTPException(
                ...
                detail="Gemini не настроен. Укажите GEMINI_API_KEY в .env",
            )

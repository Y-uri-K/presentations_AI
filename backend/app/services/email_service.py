...
def send_password_reset_code_email(*, to_email: str, code: str) -> None:
    subject = "AIDeck — восстановление пароля"
    body = (
    ...
    if settings.email_dev_mode:
        logger.info("[EMAIL_DEV_MODE] Код сброса пароля для %s: %s", to_email, code)
        print(f"[EMAIL_DEV_MODE] Password reset code for {to_email}: {code}")
        return
